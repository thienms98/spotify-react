...
export const queueSlice = createSlice({
  ...
  reducers: {
    ...
    setPlaylistAsQueue(state, action: PayloadAction<string>) {
      ...
      // fetch(`https://spotify23.p.rapidapi.com/albums/?ids=${state.uri.split(':').[2]}`, options)
      //   .then(res => res.json())
      //   .then(res => {
      //     state.list = res.albums.at(0).tracks.items;
      //   })
      const res = {
        albums: [
          {
      ...
      state.list = album.tracks.items;
    },
  },
});

export const { createQueueBySingle, addSingleToQueue, setPlaylistAsQueue } = queueSlice.actions;
